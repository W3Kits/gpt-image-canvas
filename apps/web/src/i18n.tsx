...
const zhMessages = {
  ...
  agentConfigLoading: "正在检查 Agent LLM",
  ...
  agentConfigReadyCopy: ({ model }: { model: string }) => `当前模型：${model || "未命名模型"}`,
  ...
  agentMessageRole: ({ role }: { role: "user" | "assistant" | "system" | "error" | "plan" }) =>
    ({ user: "你", assistant: "Agent", system: "状态", error: "错误", plan: "计划" })[role],
  ...
  agentTextOnlyMode: "文本模型模式",
  agentTranscriptLabel: "Agent 对话记录",
  agentVisionMode: "视觉模型模式",
...
const enMessages: I18nMessages = {
  ...
  agentConfigLoading: "Checking Agent LLM",
  ...
  agentConfigReadyCopy: ({ model }) => `Current model: ${model || "unnamed model"}`,
  ...
  agentMessageRole: ({ role }) => ({ user: "You", assistant: "Agent", system: "Status", error: "Error", plan: "Plan" })[role],
  ...
  agentTextOnlyMode: "Text-only model mode",
  agentTranscriptLabel: "Agent transcript",
  agentVisionMode: "Vision model mode",
